...
// ** Fit functor object
// ** for a line fit
// ** using ROOT example line3Dfit from Lorenzo Moneta

struct LineDistance2 {
  // the TGraph is a data member of the object
  TGraph2DErrors *fGraph;
  
  LineDistance2(TGraph2DErrors *g) : fGraph(g) {}
...
struct HelixDistance2 {
  // the TGraph is a data member of the object
  TGraph2DErrors *fGraph;
  
  HelixDistance2(TGraph2DErrors *g) : fGraph(g) {}
 
  double distance(double x,double y,double z,double ex,double ey,double ez, const double *p)
  {
...
// *****
// ** SimpleFit methods
// ** fit index data from clustering, 
// ** not real coordinates or tangent points.
// *****

void SimpleFit::init(std::unordered_map<unsigned int, std::vector<MetaInfo> >& cls)
{
  if (clusters.size())
    clusters.clear();
  clusters = cls; // working copy
  accept.clear();
  goodLineFits.clear();
  goodHelixFits.clear();
}
...
std::unordered_map<unsigned int, std::vector<MetaInfo> > SimpleFit::getClusters()
{
  std::unordered_map<unsigned int, std::vector<MetaInfo> > clustercopy;
  for (unsigned int id : accept) // store in temp container
    clustercopy[id] = clusters[id];

  clusters.clear(); // remove all rejected clusters
  unsigned int counter = 1; // start ids at 1 again
  for (auto& entry : clustercopy) {
    clusters[counter] = entry.second;
    counter++;
  }
...
bool SimpleFit::fitline(unsigned int clsid)
{
  // make the line functor objet
  LineDistance2 sdist(tgraph);
  ...
  double iniline[4] = {xyi,xysl,xzi,xzsl};
  
  fitter.SetFCN(fcn,iniline,(unsigned int)tgraph->GetN(),true);
  ...
  if (!ok)
    std::cout << "cluster " << clsid << ": failed line fit." << std::endl;
  else {
    const ROOT::Fit::FitResult & lineresult = fitter.Result();
    LineResult = FitLine(lineresult); // transferable data struct
  }
  return ok;
}


bool SimpleFit::fithelix(unsigned int clsid)
{
  ...
  // make the helix functor objet
  HelixDistance2 sdist(tgraph);
  ROOT::Math::Functor fcn(sdist,6);
  
  // set the function and the initial parameter values
  double inihelix[6] = {tx[0], ty[0], tz[0], invradius, tanlambda, bfield};
  fitter.SetFCN(fcn,inihelix,(unsigned int)tgraph->GetN(),true);

  // set step sizes different than default ones (0.3 times parameter values)
  //  for (int i = 0; i < 5; ++i) fitter.Config().ParSettings(i).SetStepSize(0.01);
  //  fitter.Config().ParSettings(5).Fix(); // B-field fixed

  
  bool ok = fitter.FitFCN();
  if (!ok)
    std::cout << "cluster " << clsid << ": failed helix fit." << std::endl;
  else {
    const ROOT::Fit::FitResult & helixresult = fitter.Result();
    HelixResult = FitHelix(helixresult); // transferable data struct
  }
  return ok;
}
...
void SimpleFit::selectClusters()
{
  ...
  for (auto& entry : clusters) {
    ...
    // using the private data tgraph for both fits
    ...
    tgraph->Clear(); // clear memory use, singleton stays
  }
}
